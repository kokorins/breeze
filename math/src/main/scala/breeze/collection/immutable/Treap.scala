...
class Treap[A, B <: AnyRef](val root: TreapNode[A, B])(override implicit val ordering: Ordering[A])
  ...
  override def size: Int = count.toInt
...
case class TreapEmptyNode[A, B <: AnyRef]()(implicit order: A => Ordered[A]) extends TreapNode[A, B] {
  ...
  def elements(t: T): Iterator[(A, Nothing)] = Iterator.empty
...
abstract class TreapFullNode[A, B <: AnyRef]() extends TreapNode[A, B] {
  ...
  def elements(t: T): Iterator[(A, B)] =
    left(t).elements(t) ++ Iterator((key, value(t))) ++ right(t).elements(t)

  def range(t: T, from: Option[A], until: Option[A]): TreapNode[A, B] = {
    if (from.isEmpty && until.isEmpty)
      return this

    if (from.isDefined && t.compare(key, from.get) < 0) return right(t).range(t, from, until)
    if (until.isDefined && t.compare(key, until.get) >= 0) return left(t).range(t, from, until)
